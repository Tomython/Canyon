...
public class AsteroidDensity : Density
{
    ...
    public override Vector3Int GetBounds()
    {
        float scaleFactor = radius * 2.0f + noiseData.amplitude * 2.0f;
        Vector3 bounds = new Vector3(scale.x * scaleFactor, scale.y * scaleFactor, scale.z * scaleFactor);
        return new Vector3Int(Mathf.RoundToInt(bounds.x), Mathf.RoundToInt(bounds.y), Mathf.RoundToInt(bounds.z));
    }
